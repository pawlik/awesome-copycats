...
mytextclock = lain.widgets.abase({
    timeout  = 60,
    cmd      = "en=`LC_TIME=en_US date +'%b'`; echo $en'/'`date +'%b %a %d E/Waw: %R'` `TZ=Asia/Vladivostok date +'A/Vlad: %R'`",
    settings = function()
...
batwidget = lain.widgets.bat({
    settings = function()
        _status = '-- ' -- we assume it's discharging
        ...
        widget:set_markup(" " .. bat_now.perc .. "% (" .. bat_now.time .. ") " .. _status)

	if bat_now.status == "Charged" then
            naughty.notify({
		text="Unplug :)", timeout = 30, position="bottom_right"
            })
        elseif tonumber(bat_now.perc) < 15 and bat_now.status == "Discharging" then 
            naughty.notify({
		text="Plug-in :D ("..bat_now.perc.."%)", timeout = 30, position="bottom_right"
            })
        end
    end
})
...
for s = 1, screen.count() do
    ...
    -- Create the wibox
    mywibox[s] = awful.wibox({ position = "top", screen = s, height = 2*18})
    ...
    -- the layout look roughly like this: 
    -- +----tworow (vertical flex)---------------------------------------------+
    -- |  +--toplayout---------------------------------------------------------+
    -- |  |left_layout(tags, runner), middle(now empty), right_layout(widgets) |
    -- |  +--------------------------------------------------------------------+
    -- |  +--bottomlayout (tasklists, aka. opened windows) --------------------+
    -- |  | mytasklist[s] - for current screen                                 |
    -- |  +--------------------------------------------------------------------+
    -- +-----------------------------------------------------------------------+

    -- Now bring it all together (with the tasklist in the middle)
    -- local layout = wibox.layout.align.horizontal()
    local tworow = wibox.layout.flex.vertical()
    ...
    bottomlayout:set_left(mytasklist[s])
    -- layout:set_middle(mytasklist[s])
    --layout:set_right(right_layout)

    tworow:add(toplayout)
    tworow:add(bottomlayout)

    mywibox[s]:set_widget(tworow)
...
globalkeys = awful.util.table.join(
    ...
    --[[ disabled because of Terminator alt-left etc
    ...
    -- ALSA volume control
    -- use `xev` to find key code strings
    awful.key({}, "XF86AudioRaiseVolume", -- Fn+F6
        function ()
